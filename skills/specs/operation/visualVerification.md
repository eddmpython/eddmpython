...
자동 검사는 지정한 모바일 폭보다 늘어난 레이아웃, 가로 넘침, 깨진 이미지, 본문 이미지의 프레임 비율과 잘림·색 필터,
브라우저 오류, 필수 요소, 노출된 HTML 주석을 잡는다. 절별 부제·주 시각물·설명 문단의 순서는
`sectionAdvisories`에 편집 권고로 남긴다. 섹션 기준은 [blogMedia.md](blogMedia.md)가 소유한다. 하지만
문장의 시각적 위계, 이미지와 설명의 연결, 지나치게 큰 여백처럼 화면을 보고 판단해야 하는 문제까지
자동 검사 통과로 대신하지 않는다.

블로그 이미지의 수동 검수는 [blogMedia.md의 제작과 검수 완료 조건](blogMedia.md#제작과-검수의-완료-조건)을
따른다. 공식 원본을 쓴 경우도 동일하며, 프레임이 정상이라는 자동 검사 결과만으로 승인하지 않는다.

캡처 직전에는 전환과 애니메이션을 멈추고 `fade-up`과 `reveal` 요소를 완료 상태로 고정한다. 자동으로
넘어가는 제품 뷰어는 처음 보였을 슬라이드를 시간으로 추측하지 않고, 계약에 적은 버튼을 직접 눌러 각
제품 링크와 문장을 확인한다.

이미지 준비를 기다리기 전에 유튜브 썸네일을 포함한 모든 이미지의 지연 로딩을 해제한다.
접근성 트리에서 숨긴 썸네일도 실제 화면에는 보이므로 캡처 준비에서 누락하지 않는다.
...
먼저 [workspace.md](workspace.md)에 따라 현재 작업의 `EDDMPYTHON_RUN_DIR`을 지정한다.
검증은 그 작업 경로의 `visual/<run-id>/`에 다음 증거를 남긴다. 빌드부터 배포까지 같은 경로를 쓴다.
