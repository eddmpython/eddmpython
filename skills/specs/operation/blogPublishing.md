...
검사기는 첫 평가와 수정본 재평가가 한 번씩 기록됐는지, 두 라운드의 평가자 수와 역할 구성이 같은지,
마지막 라운드에 새 지적이 없는지를 본다. **평가자가 몇 명이고 역할 이름이 무엇인지는 묻지 않는다.**
그 값은 스킬이 정하고 여기에 복사하면 또 갈라진다. 001, 003, 004의 네 라운드 기록은 현재 절차가
생기기 전에 실제로 돈 평가 이력이므로 지우지 않고 검사기에서 그대로 받는다.
...
때문이다. 사람 수로 세지 않던 때는 여러 역할로 네 라운드를 돌린 글이 176건에 걸렸고, 그 기준으로는
글 전체를 `kept`로 옮겨 적어야 해서 `kept`가 판단의 기록이 아니라 통과용 목록이 된다.

**재평가에서도 수렴하지 않는 경우가 있다.** 스킬은 그때 평가자를 더 붙이거나 평가를 반복하지 않고
해결하지 못한 문장과 의견 차이를 사람에게 보고하게 한다. `review.json`에는 `loop`를
`"unresolved"`로 적고 `why`에 경위를 남긴다. 첫 평가만 하고 쓰는 도피처가 되지 않게 수정본
재평가까지 기록했을 때만 받는다. 2026-08-27에 기존 절차로 네 라운드를 돌린 결과가
113, 101, 114, 113으로 줄지 않았고, 마지막 라운드가 앞선 세 라운드가
...
코드셀의 모양과 키 조작은 강의장과 함께 `site/src/codeCell.ts`와 `site/src/codeIndent.ts`를 쓴다.
실행 버튼은 코드 왼쪽에 놓고 전체 카드 테두리는 두지 않는다. 내용만큼 자라는 입력 칸과 실행 뒤에만
나타나는 출력이 본문에 이어진다. 실행 요청은 눌렀을 때의 코드를 보관하고 순서대로 처리한다.
로컬 블로그와 강의장을 실행한 뒤 `npm run shot:code-cell`로 양쪽 예제와 실제 키 입력을 확인한다.
화면 증거는 [visualVerification.md](visualVerification.md)의 정식 경로에 남기며 직접 눈으로 확인한다.
...
**브라우저 실행 칸의 파일은 브라우저 안의 가상 파일시스템에 생긴다.** 현재 셀에는 그 파일을
내 컴퓨터로 내려받는 기능이 없다. 파일을 만드는 예제라면 셀 코드는 결과를 화면에 찍는 형태로
바꾸고, 내 컴퓨터에서는 무엇이 달라지는지 `hint` 에 한 줄로 적는다. 본문의 코드
