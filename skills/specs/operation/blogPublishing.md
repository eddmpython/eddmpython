...
**10단계는 예외다.** 거기서 운영자를 기다린다. 나머지를 다 해 놓고 통과를 요청한 뒤 기다리는
것이 정상이고, 통과가 없는 채로 11단계를 시도하면 `deploy` 가 스스로 선다.

**승인 뒤에는 운영자가 다시 시키지 않아도 AI 가 나머지 단계를 이어서 끝낸다.** 2026-08-31
운영자 결정이다. 운영자의 몫은 승인 하나이고, 승인을 확인하고 진행하는 것은 AI 의 몫이다.

- 세션이 살아 있으면 `npm run check:approved` 를 읽기 전용으로 반복 확인하다가 통과가
  확인되는 즉시 11단계부터 진행한다. 이 승인을 기다리며 걸려 있던 배포 뒤 작업(예: 교안
  발행과 운영 검수)이 있으면 같은 흐름에서 끝까지 간다
- 기다리는 동안 할 일이 남아 있으면 그것을 한다
- 세션을 끝내야 하면 무엇을 통과시켜야 하는지와 승인 뒤 남은 단계를 정확히 적어 두고,
  다음 세션이 승인 확인부터 이어받는다

이 결정은 승인 문 자체를 약하게 만들지 않는다. 승인은 여전히 TTY 에서만 만들어지고 AI 는
`blog/approved.json` 을 쓰지 않는다. 바뀐 것은 승인 뒤의 대기 한 번을 없앤 것뿐이다.
