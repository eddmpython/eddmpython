...
- 관객 화면에 노트를 띄우지 않아야 할 때는 `발표자` 버튼이나 `P`로 분리 창을 연다. 분리 창은
  ...
  동기화한다. 관객 화면의 무대 폭과 좌표는 발표자 창을 열어도 바뀌지 않는다
...
- 발표자 분리 창은 `presenter=1`인 같은 글 화면이며 `BroadcastChannel`로 장면과 클릭을 동기화한다.
  새 API나 별도 교안 복사본을 만들지 않고 방 인증과 잠금 경계를 그대로 따른다
...
완성 프레임의 측정 경계다.
제품 조작 검사는 전체화면 해제와 복구, 시작과 끝 버튼 상태, `inert`와 음성 안내, 모바일 스와이프,
미디어 실패와 복구, 발표자 분리 보기와 양방향 프레임 동기화를 데스크톱과 모바일에서 함께 확인한다.
