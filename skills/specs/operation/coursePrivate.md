...
2026-08-19 에 운영자가 정했다. 유료로 돌린 네 편의 본문이 GitHub 공개 이력에 남아 있고
그것을 지우지 않는다. **커리큘럼을 계속 발전시키므로 지금 공개된 것은 곧 옛 판이 된다.**
history 재작성은 되돌릴 수 없고 이미 클론한 곳에는 어차피 남는다.

보호하는 것은 앞으로 만드는 것이다. 교안 plan 을 `course/` 로 가른 것이 그 장치다.
