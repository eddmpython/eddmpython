...
전역 `$blog-writing`은 `한 절 아래에 소제목이 하나뿐이고 같은 뜻을 되풀이하면 소제목을 지운다`고
한다. 두 규칙이 만나는 자리는 하나다. **부제가 H2 를 되풀이할 뿐이라면 그 절에는 `section` 이미지를
두지 않는다.** 부제를 억지로 만들어 이미지를 붙이지 말고, 이미지가 없는 절로 두거나 대표 이미지를
`hero` 로 계획한다. `hero` 는 절에 매이지 않아 부제를 요구하지 않는다.
