...
블로그를 쓰거나 고칠 때는 전역 `$blog-writing`을 반드시 사용한다. eddmpython의 파일 형식과 발행
순서는 아래 `operation.blogPublishing`과 `operation.blogCopy`에서 직접 확인한다. 같은 글쓰기 규칙을
저장소 스킬이나 메모리에 다시 만들지 않는다.
...
**규칙 문서를 다른 폴더에 새로 만들지 않는다.** 글쓰기 판단과 작법은 전역 `$blog-writing`,
eddmpython의 운영 계약은 이 폴더가 맡는다.
