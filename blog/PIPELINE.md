...
발행 글은 `blog/YYYYMMDD-kebab.md` 한 파일이다. 파일명 앞의 8자리 날짜와 kebab은
정렬·미디어 키용 **post id**다. frontmatter `date`는 `YYYY-MM-DD`를 유지한다. 공개 URL은
frontmatter `slug`만 쓴다 (`/blog/{slug}`). 날짜를 URL에 넣지 않는다.
