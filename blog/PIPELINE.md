...
커리큘럼은 글 목록만 담지 않는다. `modules`는 여러 차시가 함께 만드는 학습 결과를 적고,
`deliveryProfiles`는 강의 전 준비, 8시간 현장 강의, 16시간 확장 강의, 자기주도 과정에서 사용할
차시를 고른다. 블로그 전체 순서를 현장 강의 시간표에 억지로 맞추지 않고, 같은 stage id를 서로 다른
수업 구성에서 재사용한다.
...
각 stage에는 `moduleId`, `lessonType`, `studyMinutes`, `prerequisites`를 함께 적는다. `lessonType`은
개념 설명, 함께 하는 실습, 문제 해결, 프로젝트, 수행평가, 현업 전이 중 이 차시의 실제 수업 행동을
뜻한다. `prerequisites`는 이 글에서 설명하지 않고 사용할 수 있는 앞 단계만 적는다. 발행 글 하나는
stage 하나에만 연결하며, 공개 글이 커리큘럼에서 빠지거나 두 단계에 중복 연결되지 않게 검사한다.

중간에 학습 단계가 필요해지면 새 `id`를 만들고 `order`만 다시 정렬한다. 이미 발행한 파일 순번과
slug는 바꾸지 않는다. 각 stage의 `artifact`에는 독자가 남길 파일, 코드, 화면 또는 기획서를 적고,
`completionCheck`에는 성공을 확인할 값이나 행동을 적는다. 글 본문에는 `지난편`, `다음편`, `이전 글`,
`다음 글` 안내를 넣지 않는다. 각 글은 검색으로 바로 들어와도 자체 질문과 실습과 확인을 모두 끝낸다.
전체 학습 순서와 추가 단계는 `/blog`의 커리큘럼 목록만 맡는다.
...
교안에 둔다. 공개 stage id를 교안에서도 그대로 사용해 두 문서의 순서가 바뀌어도 같은 차시를 찾게
한다. 글을 쓰다가 선수 설명이 빠졌거나 실습 순서가 어색하다고 확인되면 커리큘럼을 먼저 고치고,
교안의 해당 stage id도 같은 작업에서 확인한다.
