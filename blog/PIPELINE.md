...
각 H2는 `제목 -> 섹션 이미지 -> 캡션 -> 설명 -> 예시 또는 다음 절의 다리` 순서로 설계한다.
이미지는 H2 바로 뒤에 두며, 다른 H2와 같은 이미지를 재사용하지 않는다. 제목만 읽는 독자는 글의
논리를 훑고, 이미지만 보는 독자는 장면을 기억하고, 설명을 읽는 독자는 둘 사이의 인과를 이해해야 한다.
셋 중 하나라도 같은 말을 장식적으로 반복하면 섹션을 다시 기획한다.
...
사이트는 이 줄을 브라우저 안에서 실행되는 Codaro 실습 셀로 바꾼다. 실습 셀은 한 개념을 고쳐 보고
출력을 확인하는 미리보기만 맡는다. 강검증, 진도 저장, 리액티브 문서처럼 전체 제품에서만 보장하는
기능은 셀 아래 전체 Web Run 링크로 이어 주고, 미리보기에서 제공하는 것처럼 쓰지 않는다.
