...
function hasConcreteAnchor(text) {
  ...
  return (
    MARKUP_ANCHOR.test(withoutListNumbers) ||
...
export function lintBlogStyle({ fields = {}, body, sections = [] }) {
  ...
  for (const [index, block] of proseBlocks(body).entries()) {
    ...
    const anchored = hasConcreteAnchor(block);
    if (abstractCount >= 4 && !anchored) {
      issues.push({
  ...
  for (const section of sections) {
    ...
    if (!hasConcreteAnchor(withoutMedia(section.content))) {
      issues.push({
        location: `H2 ${section.heading}`,
